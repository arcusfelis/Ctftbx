/* EXISTS AN INTERFACE PROGRAM TO MATLAB : CTFRBUD.C                          *
 *============================================================================*
 * Name of the function : bud.c (void)                                        *
 * Authors              : Emmanuel Roy - Manuel DAVY                          *
 * Date of creation     : 10 - 02 - 1999                                      *
 *----------------------------------------------------------------------------*
 * THE ALGORITHM                                                              *
 *                                                                            *
 * Given a signal to analyze in time and frequency, computes the Butterworth  *
 * Distribution (BUD) :                                                       *
 *                                                                            *
 *                    /                                                       *
 *                    | sqrt(sigma)    -|mu|*sqrt(sigma)/ |tau|               *
 *         BUD(t,f) = | -----------  e                             .../...    *
 *                    |   2 |tau |                                            *
 *                   /                                                        *
 *                                                                            *
 ...
 *                     | matrix                                               *
 * tfr.is_complex      | must be set to FALSE (a BUD tfr is real-valued)      *
 *                     |                                                      *
 ...
 *                     |                                                      *
 * BUDKernel           | variable to compute the Butterwoth Kernel            *
...

{
 ...
 /*--------------------------------------------------------------------------*/
   ...
   if (tfr.is_complex == TRUE)
    {
      printf ("bud.c : The tfr matrix must be real valued\n");
      exit(0);
    }

  if (tfr.N_freq <= 0)
    {
      printf ("bud.c : The field tfr.N_freq is not correctly set\n");
      exit(0);
    }

  if (tfr.N_time <= 0)
    {
      printf ("bud.c : The field tfr.N_time is not correctly set\n");
      exit(0);
    }

  if (ISODD(WindowT_Length) == 0)
    {
      printf ("bud.c : The time-window Length must be an ODD number\n");
      exit(0);
    }

  if (ISODD(WindowF_Length) == 0)
    {
      printf ("bud.c : The frequency-window Length must be an ODD number\n");
      exit(0);
    }
 ...
 /*--------------------------------------------------------------------------*/
  ...
  BUDKernel = (double *) ALLOC (MIN(tfr.N_freq,half_WindowF_Length) * 
  ...
  for(tau=1;tau<=taumax;tau++)
	...
	{ 
	  BUDKernel[idx(tau-1,half_WindowT_Length+mu,taumax)] 
	           = exp(-ABS(mu)*1.0/(2.0*tau/sqrt(sigma)))
  ...
  for (column = 0; column < tfr.N_time; column++)
    {
      ...
      for (tau = 1; tau <= taumax; tau++)
         ...
         for(mu=-mumin;mu<=mumax;mu++)
           {
	         normK = normK + BUDKernel[idx(tau-1,half_WindowT_Length+mu,
     ...
     if ((time<=Signal.length-tau-1)&(time>=tau)&(tau<=half_WindowF_Length))
       ...
       for(mu=-mumin;mu<=mumax;mu++)
	     {
	      normK = normK +
         BUDKernel[idx(tau-1,half_WindowT_Length+mu,
                            MIN(tfr.N_freq / 2,half_WindowF_Length))];
	    }
