...

{
  ...
  if ((nrhs < 1) || (nrhs > 3) || (nlhs < 1) || (nlhs > 3))
    mexErrMsgTxt
      ("[TFR,t,f]=CTFRRI(Signal,Time_instants,Number of frequencies)");
  ...
  tfr.is_complex = TRUE;
  RI_OUT = mxCreateDoubleMatrix (tfr.N_freq, tfr.N_time, mxCOMPLEX);
  mem_alloc_TFR (&tfr, ptr_freq_vect, ptr_time_inst, mxGetPr (RI_OUT),
		 mxGetPi (RI_OUT));
