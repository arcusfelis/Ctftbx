...

{
  ...
  if ((nrhs < 1) || (nrhs > 3) || (nlhs < 1) || (nlhs > 3))
    mexErrMsgTxt
      ("[TFR,t,f]=CTFRMH(Signal,Time_instants,Number of frequencies)");
  ...
  MH_OUT = mxCreateDoubleMatrix (tfr.N_freq, tfr.N_time, mxREAL);
  mem_alloc_TFR (&tfr, ptr_freq_vect, ptr_time_inst, mxGetPr (MH_OUT),
		 NULL);
