...

{
  ...
  if ((nrhs < 1) || (nrhs > 6) || (nlhs < 1) || (nlhs > 3))
    mexErrMsgTxt
      ("[TFR,t,f]=CTFRCW(Signal,Time_instants,Number of frequencies,Temporal Smoothing Window, Frequency Smoothing Window,SIGMA)");
  ...
  CW_OUT = mxCreateDoubleMatrix (tfr.N_freq, tfr.N_time, mxREAL);
  mem_alloc_TFR (&tfr, ptr_freq_vect, ptr_time_inst, mxGetPr (CW_OUT),
		 mxGetPi (CW_OUT));
  ...
  cw(Signal, WindowT, WindowT_Length, WindowF, WindowF_Length, sigma, tfr);
