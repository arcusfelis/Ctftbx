...

{
  ...
  if ((nrhs < 1) || (nrhs > 4) || (nlhs < 1) || (nlhs > 4))
    mexErrMsgTxt
      ("[TFR,t,f,norm]=CTFRSTFT(Signal,Time_instants,Number of frequencies,Window)");
  ...
  tfr.is_complex = TRUE;
  STFT_OUT = mxCreateDoubleMatrix (tfr.N_freq, tfr.N_time, mxCOMPLEX);
  mem_alloc_TFR (&tfr, ptr_freq_vect, ptr_time_inst, mxGetPr (STFT_OUT),
		 mxGetPi (STFT_OUT));
  ...
  stft (Signal, Window, Window_Length, tfr, norm_vector);
