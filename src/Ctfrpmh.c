...

{
  ...
  if ((nrhs < 1) || (nrhs > 4) || (nlhs < 1) || (nlhs > 3))
    mexErrMsgTxt
      ("[TFR,t,f]=CTFRPMH(Signal,Time_instants,Number of frequencies,Window)");
  ...
  PMH_OUT = mxCreateDoubleMatrix (tfr.N_freq, tfr.N_time, mxREAL);
  mem_alloc_TFR (&tfr, ptr_freq_vect, ptr_time_inst, mxGetPr (PMH_OUT),
		 NULL);
  ...
  pmh(Signal, Window, Window_Length, tfr);
