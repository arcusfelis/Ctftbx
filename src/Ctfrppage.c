...

{
  ...
  if ((nrhs < 1) || (nrhs > 4) || (nlhs < 1) || (nlhs > 3))
    mexErrMsgTxt
      ("[TFR,t,f]=CTFRPPAGE(Signal,Time_instants,Number of frequencies,Window)");
  ...
  PPAGE_OUT = mxCreateDoubleMatrix (tfr.N_freq, tfr.N_time, mxREAL);
  mem_alloc_TFR (&tfr, ptr_freq_vect, ptr_time_inst, mxGetPr (PPAGE_OUT),
		 mxGetPi (PPAGE_OUT));
  ...
  ppage(Signal, Window, Window_Length, tfr);
