...
#define RS               prhs[5]
#define MOVERN            prhs[6]

#define GRD_OUT          plhs[0]
...

{
  ...
  if ((nrhs < 1) || (nrhs > 7) || (nlhs < 1) || (nlhs > 3))
    mexErrMsgTxt
      ("[TFR,t,f]=CTFRGRD(Signal,Time_instants,Number of frequencies,Temporal Smoothing Window, Frequency Smoothing Window,Rs,MoverN)");
  ...
  /* if there are not the seven inputs ... */
  ...
  if (nrhs >= 6)		/* rs is given */
    {
      rs = (int) mxGetScalar (RS);
    }
  else
    /* rs is not given */
    {
  ...
  if (nrhs >= 7)		/* MoverN is given */
    {
      MoverN = (int) mxGetScalar (MOVERN);
    }
  else
    /* MoverN is not given */
    {
  ...
  GRD_OUT = mxCreateDoubleMatrix (tfr.N_freq, tfr.N_time, mxREAL);
  mem_alloc_TFR (&tfr, ptr_freq_vect, ptr_time_inst, mxGetPr (GRD_OUT),
		 mxGetPi (GRD_OUT));
  ...
  grd(Signal, WindowT, WindowT_Length, WindowF, WindowF_Length, rs, MoverN, tfr);
