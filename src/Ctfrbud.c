...

{
  ...
  if ((nrhs < 1) || (nrhs > 6) || (nlhs < 1) || (nlhs > 3))
    mexErrMsgTxt
      ("[TFR,t,f]=CTFRBUD(Signal,Time_instants,Number of frequencies,Temporal Smoothing Window, Frequency Smoothing Window,SIGMA)");
  ...
  /* if there are not the six inputs ... */
  ...
  if (nrhs >= 6)		/* sigma is given */
    {
      sigma = (int) mxGetScalar (SIGMA);
    }
  else
    /* sigma is not given */
    {
  ...
  BUD_OUT = mxCreateDoubleMatrix (tfr.N_freq, tfr.N_time, mxREAL);
  mem_alloc_TFR (&tfr, ptr_freq_vect, ptr_time_inst, mxGetPr (BUD_OUT),
		 mxGetPi (BUD_OUT));
  ...
  bud(Signal, WindowT, WindowT_Length, WindowF, WindowF_Length, sigma, tfr);
