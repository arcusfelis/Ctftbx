/* EXISTS AN INTERFACE PROGRAM TO MATLAB : CTFRRIDH.C                         *
 *============================================================================*
 * Name of the function : ridh.c (void)                                       *
 ...
 * Interference Distribution with Hanning kernel (RIDH) :                     *
 *                                                                            *
 *                    /                                                       *
 *                    |                  -j2pi f tau                          *
 *        RIDH(t,f) = | h(tau) R(t,tau) e            dtau                     *
 *                    |                                                       *
 *                   /                                                        *
 *                                                                            *
 *                    / +|tau|/2                                              *
 ...
 *                     | matrix                                               *
 * tfr.is_complex      | must be set to FALSE (a RIDH tfr is real-valued)     *
 *                     |                                                      *
 *----------------------------------------------------------------------------*
 * OUTPUT VARIABLES                                                           *
 * Name                |                role                                  *
 ...
 *                     |                                                      *
 * RIDHKernel          | variable to compute the RIDH Kernel                  *
...

{
 ...
 /*--------------------------------------------------------------------*/
   ...
   if (tfr.is_complex == TRUE)
    {
      printf ("ridh.c : The tfr matrix must be real valued\n");
      exit(0);
    }

  if (tfr.N_freq <= 0)
    {
      printf ("ridh.c : The field tfr.N_freq is not correctly set\n");
      exit(0);
    }

  if (tfr.N_time <= 0)
    {
      printf ("ridh.c : The field tfr.N_time is not correctly set\n");
      exit(0);
    }

  if (ISODD(WindowT_Length) == 0)
    {
      printf ("ridh.c : The time-window Length must be an ODD number\n");
      exit(0);
    }

  if (ISODD(WindowF_Length) == 0)
    {
      printf ("ridh.c : The frequency-window Length must be an ODD number\n");
      exit(0);
    }
 ...
 /* normalization of the frequency smoothing window */
  ...
  RIDHKernel = (double *) ALLOC ( WindowT_Length , sizeof(double) );
  for(row = 0 ; row < WindowT_Length ; row++)
    {
      RIDHKernel[row]=0.0;
    }
      ...
      for (tau = 1; tau <= taumax; tau++)
	{
	 ...
	 mumin=MIN(tau,half_WindowT_Length);
         ...
         for(mu = -mumin ; mu <= mumax ; mu++)
           {
	     RIDHKernel[half_WindowT_Length + mu] =
	       (1.0 + cos((double)(pi*mu)/(double)tau))
	       *WindowT[half_WindowT_Length + mu];

	     /* computation of the current kernel norm */
	     normK = normK +  RIDHKernel[half_WindowT_Length + mu];
           }
