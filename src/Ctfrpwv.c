...

{
  ...
  if ((nrhs < 1) || (nrhs > 4) || (nlhs < 1) || (nlhs > 3) )
    mexErrMsgTxt
      ("[TFR,t,f]=CTFRPWV(Signal,Time_instants,Number of frequencies,Window)");
  ...
  PWV_OUT = mxCreateDoubleMatrix (tfr.N_freq, tfr.N_time, mxREAL);
  mem_alloc_TFR (&tfr, ptr_freq_vect, ptr_time_inst, mxGetPr (PWV_OUT),
		 mxGetPi (PWV_OUT));
  ...
  pwv(Signal, Window, Window_Length, tfr);
