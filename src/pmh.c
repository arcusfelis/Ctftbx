/* EXISTS AN INTERFACE PROGRAM TO MATLAB : CTFRPMH.C                          *
 *============================================================================*
 * Name of the function : pmh.c (void)                                        *
 * Authors              : Emmanuel Roy - Manuel DAVY                          *
 * Date of creation     : 10 - 02 - 1999                                      *
 *----------------------------------------------------------------------------*
 * THE ALGORITHM                                                              *
 *                                                                            *
 * Given a signal to analyze in time and frequency, computes the Pseudo       *
 * Margenau-Hill Time-Frequency Distribution (PMH) :                          *
 *                                                                            *
 *              /                                                             *
 *              | h(tau)                                  -j2pi f tau         *
 ...
 * signal, a vector containing time instants, the number of frequency bins and*
 * a frequency smoothing window.                                              *
 *                                                                            *
 *============================================================================*
 * INPUT VARIABLES                                                            *
 * Name                |              role                                    *
 ...
 *                     | matrix                                               *
 * tfr.is_complex      | must be set to FALSE (a PMH tfr is real-valued)      *
 *                     |                                                      *
 *----------------------------------------------------------------------------*
 * OUTPUT VARIABLES                                                           *
 * Name                |                role                                  *
...

{
 ...
 /*--------------------------------------------------------------------*/
   ...
   if (tfr.is_complex == TRUE)
    {
      printf ("pmh.c : The tfr matrix must be real valued\n");
      exit(0);
    }

  if (tfr.N_freq <= 0)
    {
      printf ("pmh.c : The field tfr.N_freq is not correctly set\n");
      exit(0);
    }

  if (tfr.N_time <= 0)
    {
      printf ("pmh.c : The field tfr.N_time is not correctly set\n");
      exit(0);
    }

  if (ISODD(Window_Length) == 0)
    {
      printf ("pmh.c : The window Length must be an ODD number\n");
      exit(0);
    }
  /*--------------------------------------------------------------------*/
  ...
  for (column = 0; column < tfr.N_time; column++)
    {
      ...
      taumin = MIN( (tfr.N_freq / 2 - 1), half_Window_Length );
      taumin = MIN( taumin, (Signal.length - time - 1) );

      taumax = MIN( (tfr.N_freq / 2 - 1), half_Window_Length );
      taumax = MIN (taumax, time);
