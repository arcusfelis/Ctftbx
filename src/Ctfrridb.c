...

{
  ...
  if ((nrhs < 1) || (nrhs > 5) || (nlhs < 1)  || (nlhs > 5))
    mexErrMsgTxt
      ("[TFR,t,f]=CTFRRIDB(Signal,Time_instants,Number of frequencies,Temporal Smoothing Window, Frequency Smoothing Window)");
  ...
  RIDB_OUT = mxCreateDoubleMatrix (tfr.N_freq, tfr.N_time, mxREAL);
  mem_alloc_TFR (&tfr, ptr_freq_vect, ptr_time_inst, mxGetPr (RIDB_OUT),
		 mxGetPi (RIDB_OUT));
  ...
  ridb(Signal, WindowT, WindowT_Length, WindowF, WindowF_Length, tfr);
