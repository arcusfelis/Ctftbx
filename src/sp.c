/* EXISTS AN INTERFACE PROGRAM TO MATLAB : SPMEX.C                    *
 *====================================================================*
 * Name of the function : sp.c (void)                                 *
 ...
 *                                                                    *
 *              | /             -j 2 pi f s      |2                   *
 *    SP(t,f) = | | x(s)h*(s-t)e             ds  |                    *
 *              | /                              |                    *
 *                                                                    *
 * This function is real valued. Its computation requires a window,   *
 ...
 *                   |  in the tfr matrix                             *
 * tfr.is_complex    |  must be set to FALSE (a sp is real !)         *
 *--------------------------------------------------------------------*
 * OUTPUT VARIABLES                                                   *
 * Name              |                role                            *
 * tfr.real_part     | The output tfr matrix  (real_part)             *
 * tfr.freq_bins     | Vector of normalized frequencies where the     *
 *                   | tfr is computed                                *
 * norm_vector       | Value of the normalization factor applied at   *
 *                   |  the points of computation of the sp i.e.      *
 *                   |  tfr.time_instants                             *
 *--------------------------------------------------------------------*
 * INTERNAL VARIABLES                                                 *
 * Name              |                 role                           *
 ...
 *                   |                                                *
 * half_Window_Length| half of the frequency smoothing window         *
...

{
 ...
 /*--------------------------------------------------------------------*/
   ...
   if (tfr.is_complex == TRUE)
    {
      printf ("sp.c : The tfr matrix must be real valued\n");
      exit(0);
    }

  if (tfr.N_freq <= 0)
    {
      printf ("sp.c : The field tfr.N_freq is not correctly set\n");
      exit(0);
    }

  if (tfr.N_time <= 0)
    {
      printf ("sp.c : The field tfr.N_time is not correctly set\n");
      exit(0);
    }
 ...
 /*--------------------------------------------------------------------*/
  ...
  if (ISODD(Window_Length) == 0)
    {
      printf ("sp.c : The window Length must be an ODD number\n");
      exit(0);
    }
 ...
 /*--------------------------------------------------------------------*/
  for (column = 0; column < tfr.N_time; column++)
      ...
      for (row = 0; row < tfr.N_freq; row++)
	{
      ...
      /* time instants of interest to compute the sp */
	...
	{
	  inter = inter + sqr (Window[half_Window_Length + row]);
	}
      normh = sqrt (inter);
      ...
      /* the first half of the fft is put in the sp matrix  */
      for (row = 0; row < tfr.N_freq; row++)
	{
