...

{
  ...
  if ((nrhs < 1) || (nrhs > 3) || (nlhs<1) || (nlhs>3))
    mexErrMsgTxt
      ("[TFR,t,f]=CTFRWV(Signal,Time_instants,Number of frequencies)");
  ...
  WV_OUT = mxCreateDoubleMatrix (tfr.N_freq, tfr.N_time, mxREAL);
  mem_alloc_TFR (&tfr, ptr_freq_vect, ptr_time_inst, mxGetPr (WV_OUT),
		 mxGetPi (WV_OUT));
