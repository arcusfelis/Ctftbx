...

{
  ...
  if ((nrhs < 1) || (nrhs > 5) || (nlhs < 1) || (nlhs > 3))
    mexErrMsgTxt
      ("[TFR,t,f]=CTFRRIDT(Signal,Time_instants,Number of frequencies,Temporal Smoothing Window, Frequency Smoothing Window)");
  ...
  RIDT_OUT = mxCreateDoubleMatrix (tfr.N_freq, tfr.N_time, mxREAL);
  mem_alloc_TFR (&tfr, ptr_freq_vect, ptr_time_inst, mxGetPr (RIDT_OUT),
		 mxGetPi (RIDT_OUT));
  ...
  ridt(Signal, WindowT, WindowT_Length, WindowF, WindowF_Length, tfr);
